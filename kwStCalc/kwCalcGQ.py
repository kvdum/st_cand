...
from PyQt5.QtWidgets import QApplication, QMainWindow, QSplitter, QWidget,\
    ...
    QScrollArea, QSizePolicy, QAbstractSpinBox, QComboBox, QMessageBox
    
from kwConsole import KwConsoleBrowser
from antTeploMonitorDyblanu import (TargetGraph, get_figure_data, 
                                    get_show_own_frame_params, calculate)
...
class KwMainWindow(QMainWindow):
    ...
    def __createFigureCanvas(self):
        ...
        graphCanvas = FigureCanvas(Figure(**get_figure_data()))
        graphCanvas.setToolTip('Подвійний клік мишею по області графіку вікдриває '
                               'цей графік у новому вікні для кращого масштабування')
        if hasattr(self, '_KwMainWindow__graphNavigationToolbar'):
            self.__graphNavigationToolbar.canvas = graphCanvas
    ...
    def calc_script(self):
        ...
        try:
            ...
            calculate(t_z_h5=city_t_z_h5, q_0=self.__q_0Edit.value, 
              V_bud=self.__V_budEdit.value, 
              N=self.__NEdit.value(), t_gv=self.__t_gvEdit.value(),
              a_gv=self.__a_gvEdit.value(), fi=city_phi, f_z=self.__f_zEdit.value,
              vartheta=self.__varthetaEdit.value, S_0=self.__S_0Edit.value, 
              F_r=self.__F_rEdit.value, eta_0=self.__eta_0Edit.value, 
              UL=self.__ULEdit.value, a=self.__aEdit.value, b=self.__bEdit.value, 
              c_pa=self.__c_paEdit.value, tip_tp='WW110', P_tp=self.__P_tpEdit.value, 
              epsilon_tp=self.__epsilon_tpEdit.value,
              epsilon_el=self.__epsilon_elEdit.value, P_el=self.__P_elEdit.value, 
              t_co_1=self.__t_co_1Edit.value(), t_co_2=self.__t_co_2Edit.value(), 
              eta_K=self.__eta_KEdit.value, Q_n_r=self.__Q_n_rEdit.value,
              c_gaz=self.__c_gazEdit.value, c_el=self.__c_elEdit.value, 
              q_gru=self.__q_gruEdit.value, d=self.__d_gruEdit.value(), 
              l_0=self.__l_0_gruEdit.value, data_t_n_s=city_data_t_n_s,
              fname_climate=os.path.join(SCRIPT_FOLDER, 'antStepClimaData.txt'),
              target_graph=TargetGraph.show, show_own_frame=False,
              figure_embedded=self.__graphCanvas.figure)
