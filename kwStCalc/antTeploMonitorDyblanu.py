...
def calculate(t_z_h5, q_0, V_bud, N, t_gv, a_gv, fi, f_z, vartheta, S_0, F_r, eta_0,
              UL, a, b, c_pa, tip_tp, P_tp, epsilon_tp, epsilon_el, P_el, t_co_1,
              t_co_2, eta_K, Q_n_r, c_gaz, c_el, q_gru, d, l_0, 
              data_t_n_s=None, data_t_hv=None, data_t_gru=None, fname_climate=None,
              target_graph=TargetGraph.show, show_own_frame=True, 
    ...
    print('Розрахунок виконується о: {}.'.format(datetime.strftime(datetime.now(),
                                                              '%d.%m.%Y %H:%M:%S')))    
    ...
    if target_graph != TargetGraph.no_showsave:
      ...
      #767.2222,  846.9444,  898.3333,  920.5556,  936.1111])
        ...
        ax.set_title(r'Помісячний виробіток теплової енергії СГК і теплове '
                     'навантаження СГВ', y=view_graph_params['ax_title_y'])
        ax.set_xlabel('Місяці', x=1, ha="right")
        ax.set_ylabel(r'$\mathrm{Q_{i}, кВт\cdot год}$', y=1.025, 
                      ha=view_graph_params['ax_ha'], va='bottom', rotation=0)
        ax.set_xlim(mis[0], mis[-1]+1)
        ax.set_xticks(np.r_[mis, 13])
        ax.spines['top'].set_color(grid_color)
        ax.spines['right'].set_color(grid_color)
        ax.grid(color=grid_color)
        ax.legend(loc=3, ncol=5, mode='expand', bbox_to_anchor=(.05, 1.08, .9, .102),
...
if __name__ == '__main__':
    calculate(t_z_h5=-22, q_0=0.5, V_bud=480, N=6, t_gv=45, a_gv=35, fi=48, f_z=0.8,
              vartheta=1.7, S_0=1.723, F_r=1, eta_0=0.813, UL=4.6, a=0.007, 
              b=1.27*10**(-5), c_pa=3.16, tip_tp='WW110', P_tp=14.1, epsilon_tp=5.5,
              epsilon_el=0.88, P_el=2.6, t_co_1=35, t_co_2=30, eta_K=0.93, Q_n_r=35600,
              c_gaz=0.55, c_el=0.25, q_gru=21, d=25, l_0=1.7, 
              target_graph=TargetGraph.show_and_save, show_own_frame=True)